    ...
    public Step shareStep() {
        return stepBuilderFactory.get("shareStep")
                .tasklet((contribution, chunkContext) -> {
                    ...
                    stepExecutionContext.putString("stepKey", "step executaion context"); // 같은 Step 끼리 공유 가능(다음 스텝에서 공유 안됨)

                    JobExecution jobExecution = stepExecution.getJobExecution();
                    JobInstance jobInstance = jobExecution.getJobInstance();
                    ExecutionContext jobExecutionContext = jobExecution.getExecutionContext();
                    jobExecutionContext.putString("jobKey", "job execution context"); // 같은 job 끼리 공유 가능
